...
- Fixed products created by Bullwheel being written to Ascend with a NULL `ModifierLocationID` and `ConcurrencyToken`, which made them invisible in the item grid of any purchase order that referenced them — an imported order would show a handful of lines and a small total while its header still counted every one. Virtual DocTypes can now declare `INSERT_DEFAULTS` for Ascend columns they surface no field for; a new "Default Location" setting in Bullwheel Settings supplies the location, and `bench backfill-insert-defaults` repairs products already created. (`NonInventory` was also NULL on affected products and is now defaulted too, but was not the cause of the missing lines.)
